...
//			auteurGenApiService.l("import ", classPackageName, ".", classSimpleName, "ApiServiceVertxEBProxy;");
	...
	//					tl(1, "}");
					...
					if(classApiMethod.contains("PATCH")) {
						...
						tl(5, "a -> ", str_define(languageName), "", classApiMethod, classSimpleName, "(a).compose(");
						tl(6, "b -> ", str_index(languageName), "", classApiMethod, classSimpleName, "(b)");
						tl(5, ")");
						tl(4, ")");
						tl(3, ");");
						tl(2, "});");
						...
						tl(1, "}");
						l();
						tl(1, "public Future<", classSimpleName, "> ", str_define(languageName), "", classApiMethod, classSimpleName, "(", classSimpleName, " o) {");
						tl(2, "Future<", classSimpleName, "> future = Future.future();");
						tl(2, "try {");
						tl(3, "", classPartsSiteRequest.simpleName(languageName), " ", str_siteRequest(languageName), " = o.get", str_SiteRequest(languageName), "_();");
						tl(3, "SQLConnection ", str_sqlConnection(languageName), " = ", str_siteRequest(languageName), ".get", str_SqlConnection(languageName), "();");
						...
						tl(1, "}");
						l();
						tl(1, "public Future<Void> ", str_index(languageName), "", classApiMethod, classSimpleName, "(", classSimpleName, " o) {");
						tl(2, "Future<Void> future = Future.future();");
						tl(2, "try {");
						tl(3, "o.", str_initDeep(languageName), "", str_PourClasse(languageName), "(o.get", str_SiteRequest(languageName), "_());");
...
//			tl(8, "}");
			...
			tl(3, "-> {");
			tl(4, "if(", str_attribute(languageName), "Async.succeeded()) {");
			tl(5, "if(", str_attribute(languageName), "Async.result() != null) {");
			tl(6, "for(JsonArray definition : ", str_attribute(languageName), "Async.result().getResults()) {");
			tl(7, "o.", str_attribute(languageName), "", str_PourClasse(languageName), "(definition.getString(0), definition.getString(1));");
			tl(6, "}");
			tl(5, "}");
			tl(5, "", str_eventHandler(languageName), ".handle(Future.succeededFuture());");
			tl(4, "} else {");
			tl(5, "", str_eventHandler(languageName), ".handle(Future.failedFuture(", str_attribute(languageName), "Async.cause()));");
			tl(4, "}");
			tl(3, "});");
			tl(2, "} catch(Exception e) {");
			tl(3, "", str_eventHandler(languageName), ".handle(Future.failedFuture(e));");
