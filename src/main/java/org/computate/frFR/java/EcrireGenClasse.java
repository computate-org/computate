...
//				tl(1, "}");
	...
	//							t(tIndex + 4).l("} ", classePrefixe, "g(\"span\");");
			...
			if(entiteAttribuer && !classesNomSimpleFacetFor.contains(entiteAttribuerNomSimple)) {
				...
				wIndexerFacetFor.tl(7, "futures.add(", str_listeRecherche(langueNom), "2.", str_promesseLoin(langueNom), str_ListeRecherche(langueNom), "(", str_requeteSite(langueNom), ").onSuccess(b -> {");
				...
				wIndexerFacetFor.tl(9, entiteAttribuerNomSimpleApiServiceImpl, " service = new ", entiteAttribuerNomSimpleApiServiceImpl, "(eventBus, config, ", str_executeurTravailleur(langueNom), ", pgPool, ", str_clientWeb(langueNom), activerOpenIdConnect ? ", oauth2AuthenticationProvider, authorizationProvider" : "", ");");
				wIndexerFacetFor.tl(9, classePartsRequeteSite.nomSimple(langueNom), " ", str_requeteSite(langueNom), "2 = ", str_generer(langueNom), classePartsRequeteSite.nomSimple(langueNom), "(", str_requeteSite(langueNom), ".get", str_Utilisateur(langueNom), "(), ", str_requeteSite(langueNom), ".get", str_RequeteService(langueNom), "(), new JsonObject());");
				wIndexerFacetFor.tl(9, classePartsRequeteApi.nomSimple(langueNom), " ", str_requeteApi(langueNom), "2 = new ", classePartsRequeteApi.nomSimple(langueNom), "();");
				wIndexerFacetFor.tl(9, str_requeteApi(langueNom), "2.setRows(1);");
				wIndexerFacetFor.tl(9, str_requeteApi(langueNom), "2.setNumFound(1l);");
				wIndexerFacetFor.tl(9, str_requeteApi(langueNom), "2.setNumPATCH(0L);");
				wIndexerFacetFor.tl(9, str_requeteApi(langueNom), "2.", str_initLoin(langueNom), classePartsRequeteApi.nomSimple(langueNom), "(", str_requeteSite(langueNom), "2);");
				wIndexerFacetFor.tl(9, str_requeteSite(langueNom), "2.set", str_RequeteApi(langueNom), "_(", str_requeteApi(langueNom), "2);");
				wIndexerFacetFor.tl(9, "eventBus.publish(\"websocket", entiteAttribuerNomSimple, "\", JsonObject.mapFrom(", str_requeteApi(langueNom), "2).toString());");
				wIndexerFacetFor.l();
				wIndexerFacetFor.tl(9, "o2.set", StringUtils.capitalize(classeVarClePrimaire), "(", classeVarClePrimaire, "2);");
				wIndexerFacetFor.tl(9, "o2.set", str_RequeteSite(langueNom), "_(", str_requeteSite(langueNom), "2);");
				wIndexerFacetFor.tl(9, "service.patch", entiteAttribuerNomSimple, "Future(o2, false).onFailure(ex -> {");
				wIndexerFacetFor.tl(10, "LOG.error(String.format(\"", entiteAttribuerNomSimple, " %s ", str_a_échoué(langueNom), ". \", ", classeVarClePrimaire, "2), ex);");
//				wIndexerFacetFor.tl(10, str_gestionnaireEvenements(langueNom), ".handle(Future.failedFuture(ex));");
