...
public class EcrirePageClasse extends EcrireApiClasse {
	...
	public Boolean ecrireFormEntite(String langueNom, ToutEcrivain wForm, String classeApiMethodeMethode) {
		...
		if(entiteHtml) {
			...
			wForm.t(3).l("{{> \"html", entiteVarCapitalise, "\" ", str_classeApiMethodeMethode(langueNom), "=\"", classeApiMethodeMethode, "\"}}");
		}
		return resultat;
	}
...
//				wEntites.tl(1, "}");
			...
			if(classePageSuperNomSimple != null)
				wEntites.tl(1, "@Override");
			wEntites.tl(1, "protected void _", str_roles(langueNom), "(List<String> l) {");
			wEntites.tl(2, "if(", str_requeteSite(langueNom), "_ != null) {");
			wEntites.tl(3, "l.addAll(Stream.concat(", str_requeteSite(langueNom), "_.get", str_UtilisateurRolesRessource(langueNom), "().stream(), ", str_requeteSite(langueNom), "_.get", str_UtilisateurRolesRoyaume(langueNom), "().stream()).distinct().collect(Collectors.toList()));");
			wEntites.tl(2, "}");
			wEntites.tl(1, "}");
			if(classeRolesTrouves) {
				...
				if(classePageSuperNomSimple != null)
					wEntites.tl(1, "@Override");
				wEntites.tl(1, "protected void _", str_rolesRequis(langueNom), "(List<String> l) {");
				wEntites.tl(2, "l.addAll(Arrays.asList(\"", StringUtils.join(classeRoles, "\", \""), "\"));");
				wEntites.tl(1, "}");
			}
			if(classeRoleLiresTrouves) {
				...
				if(classePageSuperNomSimple != null)
					wEntites.tl(1, "@Override");
				wEntites.tl(1, "protected void _", str_rolesPourLires(langueNom), "(List<String> l) {");
				wEntites.tl(2, "l.addAll(Arrays.asList(\"", StringUtils.join(classeRoleLires, "\", \""), "\"));");
				wEntites.tl(1, "}");
			}
			if(activerRoleAdmin) {
				wEntites.l();
				if(classePageSuperNomSimple != null)
					wEntites.tl(1, "@Override");
				wEntites.tl(1, "protected void _", str_authRolesAdmin(langueNom), "(List<String> l) {");
				wEntites.tl(2, "l.addAll(Arrays.asList(\"", StringUtils.join(authRolesAdmin, "\", \""), "\"));");
				wEntites.tl(1, "}");
			}
			...
			wEntites.tl(2, "Long ", str_debut(langueNom), "Num = ", str_debut(langueNom), " + 1L;");
			...
			wEntites.tl(2, "Long pagination", str_Debut(langueNom), " = ", str_debut(langueNom), " - 10L * ", str_lignes(langueNom), ";");
			wEntites.tl(2, "if(pagination", str_Debut(langueNom), " < 0L)");
			wEntites.tl(3, "pagination", str_Debut(langueNom), " = 0L;");
			wEntites.tl(2, "Long pagination", str_Fin(langueNom), " = ", str_debut(langueNom), " + 10L * ", str_lignes(langueNom), ";");
			...
			wEntites.l();
			wEntites.tl(2, "pagination.put(\"1L\", 1L);");
			wEntites.tl(2, "pagination.put(\"0L\", 0L);");
			...
			wEntites.tl(2, "pagination.put(\"", str_lignes(langueNom), str_Precedent(langueNom), "\", ", str_lignes(langueNom), " / 2);");
			wEntites.tl(2, "pagination.put(\"", str_lignes(langueNom), str_Prochaine(langueNom), "\", ", str_lignes(langueNom), " * 2);");
			...
			wEntites.tl(3, "pagination.put(\"page", str_Prochaine(langueNom), "\", new JsonObject().put(\"", str_debut(langueNom), "\", ", str_debut(langueNom), " + ", str_lignes(langueNom), ").put(\"page", str_Numero(langueNom), "\", ", str_debut(langueNom), " + ", str_lignes(langueNom), " + 1L));");
...
//												}
			...
			l();
			tl(0, "{{#*inline \"htmlHead", classeGenPageNomSimple, "\"}}");
			tl(0, "{{> \"htmScripts", classeGenPageNomSimple, "\"}}");
			tl(0, "{{> \"htmStyle", classeGenPageNomSimple, "\"}}");
			tl(0, "{{/inline}}");

			l();
			tl(0, "{{#*inline \"htmStyle", classeGenPageNomSimple, "\"}}");
			tl(0, "{{/inline}}");

			l();
			tl(0, "{{#*inline \"htmScripts", classeGenPageNomSimple, "\"}}");
			...
			l();
			if(!classePageSimple) {
				tl(0, "{{#*inline \"htmScript", classeGenPageNomSimple, "\"}}");
...
//					tl(2, "tl(1, ", q("});"), ");");
				...
				tl(0, "{{/inline}}");
				l();
				tl(0, "{{#*inline \"htmlFormPage", classeNomSimple, "\"}}");
				s(wFormPage);
				tl(0, "{{/inline}}");
				l();
				tl(0, "{{#*inline \"htmlFormPOST", classeNomSimple, "\"}}");
				s(wFormPOST);
				tl(0, "{{/inline}}");
				l();
				tl(0, "{{#*inline \"htmlFormPUTImport", classeNomSimple, "\"}}");
				tl(2, "<div class=\"w3-cell-row \">");
				tl(3, "<textarea");
				tl(4, "class=\"", "PUTImport_", str_liste(langueNom), " w3-input w3-border \"");
				tl(4, "style=\"height: 400px; \"");
				tl(4, "placeholder=\"{ '", str_liste(langueNom), "': [ { 'pk': ... , '", str_sauvegardes(langueNom), "': [ ... ] }, ... ] }\"");
				tl(4, "></textarea>");
				tl(2, "</div>");
				tl(0, "{{/inline}}");
				l();
				tl(0, "{{#*inline \"htmlForm", str_PUTFusion(langueNom), classeNomSimple, "\"}}");
				tl(2, "<div class=\"w3-cell-row \">");
				tl(3, "<textarea");
				tl(4, "class=\"", "PUT", str_PUTFusion(langueNom), "_", str_liste(langueNom), " w3-input w3-border \"");
				tl(4, "style=\"height: 400px; \"");
				tl(4, "placeholder=\"{ '", str_liste(langueNom), "': [ { 'pk': ... , '", str_sauvegardes(langueNom), "': [ ... ] }, ... ] }\"");
				tl(4, "></textarea>");
				tl(2, "</div>");
				tl(0, "{{/inline}}");
				l();
				tl(0, "{{#*inline \"htmlForm", str_PUTCopie(langueNom), classeNomSimple, "\"}}");
				s(wFormPUTCopie);
				tl(0, "{{/inline}}");
				l();
				tl(0, "{{#*inline \"htmlFormPATCH", classeNomSimple, "\"}}");
				s(wFormPATCH);
				tl(0, "{{/inline}}");
				l();
				tl(0, "{{#*inline \"htmlForm", str_Recherche(langueNom), classeNomSimple, "\"}}");
				s(wFormRecherche);
				tl(0, "{{/inline}}");
			}
			l();
			t(0, "{{#*inline \"htmlUrl", classeNomSimple, "\"}}");
			...
			l("{{/inline}}");
			l();
			tl(0, "{{#*inline \"htmlBody", classeGenPageNomSimple, "\"}}");
			if(classePageSimple) {
//				l();
...
//				l();
				...
				tl(3, "{{#eq ", str_liste(langueNom), classeNomSimple, "_.length 1}}");
...
//					tl(5, "<div class=\"w3-padding-16 w3-card-4 w3-light-grey \">");
				...
				tl(8, "<a href=\"{{htmlUrl", classeNomSimple, "}}?start={{pagination.page", str_Precedent(langueNom), ".", str_debut(langueNom), "}}&rows={{pagination.", str_lignes(langueNom), "}}\">");
				...
				tl(8, "<a href=\"{{htmlUrl", classeNomSimple, "}}?start={{pagination.", str_debut(langueNom), "}}&rows={{ pagination.", str_lignes(langueNom), str_Precedent(langueNom), " }}\">");
				...
				tl(7, "{{/gte}}");
				l();
				tl(7, "<a href=\"{{htmlUrl", classeNomSimple, "}}?start={{pagination.", str_debut(langueNom), "}}&rows={{ pagination.", str_lignes(langueNom), str_Prochaine(langueNom), " }}\">");
				tl(8, "<i class=\"fas fa-plus-square \"></i>");
				tl(7, "</a>");
				l();
				tl(7, "{{#if pagination.page", str_Prochaine(langueNom), "}}");
				tl(8, "<a href=\"{{htmlUrl", classeNomSimple, "}}?start={{pagination.page", str_Prochaine(langueNom), ".", str_debut(langueNom), "}}&rows={{pagination.", str_lignes(langueNom), "}}\">");
				...
				tl(6, "{{> \"table1", classeGenPageNomSimple, "\"}}");
				tl(5, "</div>");
				tl(4, "{{/eq}}");
				tl(3, "{{else}}");
...
//					t(3).bgl("h1");
				...
				tl(6, "{{/if}}");
				tl(6, "<span class=\"\">{{pageH1}}</span>");
				tl(5, "</a>");
				tl(4, "</h1>");

//					tl(4, "<div class=\"w3-padding-16 w3-card-4 w3-light-grey \">");
				tl(4, "<div class=\"\">");
				...
				tl(5, "{{> \"table1", classeGenPageNomSimple, "\"}}");
				tl(4, "</div>");
				tl(3, "{{/eq}}");
				tl(2, "{{else}}");
				tl(3, "<h1>");
...
//					tl(2, "{{# if ", str_liste(langueNom), classeNomSimple, "_ != null && ", str_liste(langueNom), classeNomSimple, "_.size() == 1 && params.getJsonObject(\"query\").getString(\"q\").equals(\"*:*\") && params.getJsonObject(\"query\").getJsonArray(\"fq\") == null) {");
				tl(2, "{{#eq ", str_liste(langueNom), classeNomSimple, "_.length 1}}");
				...
				if(classeVarClePrimaire != null) {
					l();
					tl(6, "{{#if ", classeVarClePrimaire, "}}");
					tl(7, "<form action=\"", classeApiUri, "\" id=\"", classeNomSimple, "Form\" style=\"display: inline-block; width: 100%; \" onsubmit=\"event.preventDefault(); return false; \">");
					tl(8, "<input");
					tl(8, "name=\"", classeVarClePrimaire, "\"");
					tl(8, "class=\"", str_valeur(langueNom), StringUtils.capitalize(classeVarClePrimaire), "\"");
					tl(8, "type=\"hidden\"");
					tl(8, "value=\"{{", classeVarClePrimaire, "}}\"");
					tl(8, "/>");
					tl(8, "<input").l();
					tl(9, "name=\"focusId\"");
					tl(9, "type=\"hidden\"");
...
//				tl(2, "Map<String, Map<String, List<String>>> highlighting = ", str_liste(langueNom), classeNomSimple, "_.getQueryResponse().getHighlighting();");
				tl(2, "{{#each ", str_liste(langueNom), classeNomSimple, "_}}");
//				TODO
...
//				tl(3, "SimpleOrderedMap facets = (SimpleOrderedMap)Optional.ofNullable(", str_liste(langueNom), classeNomSimple, "_.getQueryResponse()).map(QueryResponse::getResponse).map(r -> r.get(\"facets\")).orElse(new SimpleOrderedMap());");
				...
				s(wGetters);
				l();
				tl(0, "{{#*inline \"htmlBodyForms", classeGenPageNomSimple, "\"}}");
				if(!classePageSimple) {

					tl(2, "{{#ifContainsAnyRoles roles ", str_rolesRequis(langueNom), "}}");

					// refraîchir 1 //
					tl(3, "{{#eq ", str_liste(langueNom), classeNomSimple, "_.length 1}}");
					tl(4, "<button");
					tl(5, "class=\"w3-btn w3-round w3-border w3-border-black w3-ripple w3-padding w3-", contexteCouleur, " \"");
					tl(6, "id=\"", str_recharger(langueNom), StringUtils.trim(StringUtils.capitalize(contexteCe)), classeGenPageNomSimple, "\"");
					tl(6, "onclick=\"patch", classeNomSimple, "Vals( [ {name: 'fq', value: '", classeVarClePrimaire, ":' + \" + ", str_requeteSite(langueNom), "_.get", str_Requete(langueNom), "", StringUtils.capitalize(classeVarClePrimaire), "() + \" } ], {}, function() { ", str_ajouterLueur(langueNom), "($('#", str_recharger(langueNom), StringUtils.trim(StringUtils.capitalize(contexteCe)), classeGenPageNomSimple, "')); }, function() { ", str_ajouterErreur(langueNom), "($('#", str_recharger(langueNom), StringUtils.trim(StringUtils.capitalize(contexteCe)), classeGenPageNomSimple, "')); }); return false; \">");
					tl(6, "<i class=\"fas fa-sync-alt \"></i>");
					tl(5, str_recharger(langueNom), " ", contexteCeNom);
					tl(4, "</button>");
					tl(3, "{{/eq}}");

					tl(2, "{{/ifContainsAnyRoles}}");
					if(activerRoleAdmin) {
						tl(2, "{{#ifContainsAnyRoles roles ", str_authRolesAdmin(langueNom), "}}");
					}
					...
					for(String classeApiMethode : classeApiMethodes) {
						...
						if("application/json".equals(classeApiTypeMediaMethode) && (classeApiMethode.equals("PATCH") || classeApiMethode.equals("POST") || classeApiMethode.equals(str_PUTCopie(langueNom)) || classeApiMethode.equals(str_PUTFusion(langueNom)) || classeApiMethode.equals("PUTImport"))) {
							...
							tl(3 + tab, "<button");
							tl(4 + tab, "class=\"w3-btn w3-round w3-border w3-border-black w3-ripple w3-padding w3-", contexteCouleur, " \"");
							tl(4 + tab, "onclick=\"$('#", classeApiOperationIdMethode, str_Modale(langueNom), "').show(); \"");
							tl(4 + tab, ">");

								if(classeApiMethodeMethode.contains("POST"))
									tl(4 + tab, "<i class=\"fas fa-file-plus \"></i>");
								else if(classeApiMethodeMethode.contains("PATCH"))
									tl(4 + tab, "<i class=\"fas fa-edit \"></i>");
								else if(classeApiMethode.contains("PUTImport"))
									tl(4 + tab, "<i class=\"fas fa-file-import \"></i>");
								else if(classeApiMethode.contains(str_PUTFusion(langueNom)))
									tl(4 + tab, "<i class=\"fas fa-code-merge \"></i>");
								else if(classeApiMethode.contains(str_PUTCopie(langueNom)))
									tl(4 + tab, "<i class=\"fas fa-copy \"></i>");

								tl(4 + tab, methodeTitreValeurs);
							tl(3 + tab, "</button>");
							{ tl(3 + tab, "<div id=\"", classeApiOperationIdMethode, str_Modale(langueNom), " \" class=\"w3-modal w3-padding-32\">");
								{ tl(4 + tab, "<div class=\"w3-modal-content \">");
									{ tl(5 + tab, "<div class=\"w3-card-4 \">");
										{ tl(6 + tab, "<header class=\"w3-container w3-", contexteCouleur, "\">");
											tl(7 + tab, "<span class=\"w3-button w3-display-topright \" onclick=\"$('#", classeApiOperationIdMethode, str_Modale(langueNom), "').hide(); \">×</span>");
											tl(7 + tab, "<h2 class=\"w3-padding \">", methodeTitreValeurs, "</h2>");
										} tl(6 + tab, "</header>");
			
										{ tl(6 + tab, "<div class=\"w3-container \" id=", classeApiOperationIdMethode, str_FormulaireValeurs(classePageLangueNom), ">");
//											TODO
...
//											tl(7+ tab, "o.set", str_RequeteSite(langueNom), "_(", str_requeteSite(langueNom), "_);");
											if("PATCH".equals(classeApiMethodeMethode) || str_PUTFusion(langueNom).equals(classeApiMethodeMethode) || str_PUTCopie(langueNom).equals(classeApiMethodeMethode) || "PUTImport".equals(classeApiMethodeMethode)) {
												...
												if("POST".equals(classeApiMethodeMethode))
													...
													tl(8 + tab, "onclick=\"", classeApiOperationIdMethode, "\"");
				
												tl(8 + tab, ">", methodeTitreValeurs, "</button>");
												l();
											}
											else {
												...
												if("DELETE".equals(classeApiMethodeMethode))
													...
													tl(8 + tab, "{{> \"htmlForm", str_PUTCopie(langueNom), classeNomSimple, "\" o=o}}");
												else
					...
					tl(2, "{{> \"html", str_Suggere(langueNom), classeGenPageNomSimple, "\"}}");
				}
				tl(0, "{{/inline}}");
			}
			l();
...
//				}
//				tl(1, "**/");
				tl(0, "{{#*inline \"html", str_Suggere(langueNom), classeGenPageNomSimple, "\"}}");
...
//						t(4, "}").l();
				...
				tl(4, "{{#if ", str_liste(langueNom), classeNomSimple, "_}}");
				tl(5, "value=\"{{query2}}\"");
				tl(4, "{{/if}}");
				tl(4, "/>");
				tl(4, "<button");
...
//				tl(2, "}");
				...
				if(classeMethodeVars.contains("htmlBody" + str_Court(langueNom))) {
					if(classePageSuperNomSimple != null)
						tl(1, "@Override");
					tl(0, "{{#*inline \"htmlBodyCourt", classeGenPageNomSimple, "\"}}");
					l();
...
//					auteurPageJs.tl(3, "}");
			...
			l("{{/inline}}");
			l("{{#*inline \"htmlBody\"}}");
			l("{{> \"htmlBody", classeGenPageNomSimple, "\"}}");
			l("{{/inline}}");
...
//									wTd.tl(6, "<a href=\"{{uri}}\")>");
//									if(contexteIconeGroupe != null && contexteIconeNom != null && BooleanUtils.isTrue(entiteVarTitre))
//										wTd.tl(7, "<i").da("class", "fa", StringUtils.substring(contexteIconeGroupe, 0, 1), " fa-", contexteIconeNom, " ></i>");
//									wTd.tl(7, "<span>;");
...
//						t(6).e("i class=\"fas fa-sync-alt \"></i>");
...
//
//									if(classeApiMethodeMethode.contains("POST"))
//										t(4 + tab).e("i class=\"fas fa-file-plus \"></i>");
//									else if(classeApiMethodeMethode.contains("PATCH"))
//										t(4 + tab).e("i class=\"fas fa-edit \"></i>");
//									else if(classeApiMethode.contains("PUTImport"))
//										t(4 + tab).e("i class=\"fas fa-file-import \"></i>");
//									else if(classeApiMethode.contains(str_PUTFusion(langueNom)))
//										t(4 + tab).e("i class=\"fas fa-code-merge \"></i>");
//									else if(classeApiMethode.contains(str_PUTCopie(langueNom)))
//										t(4 + tab).e("i class=\"fas fa-copy \"></i>");
//
...
//					dfl();
//					t(7).s("p.").e("i class=\"fas fa-sync-alt \"></i>");
...
//					tl(5, " onclick=\"\"window.location.href = '", classePageUriMethode + "?q=\", query1, \":' + encodeURIComponent(this.previousElementSibling.value) + '\", fqs, sorts, \"&start=\", start2, \"&rows=\", rows1, \"'; \" ");
//					t(5).dfl();
//					t(5).s("p.").e("i class=\"fas fa-search \"></i>");
//					t(4).s("} p.").gl("button");
//	
//					l();
//					t(3).s("} p.").gl("div");
...
//					if(contexteIconeGroupe != null && contexteIconeNom != null)
//						t(5).s("p.").e("i").da("class", "fa", StringUtils.substring(contexteIconeGroupe, 0, 1), " fa-", contexteIconeNom, " ></i>");
