				...
				//				tl(4, "if(", q(entiteVar, "_stored", entiteSuffixeType), ".equals(entiteVarStocke)) {");
								if(StringUtils.equalsAny(entiteNomCanonique, VAL_nomCanoniqueList, VAL_nomCanoniqueArrayList, VAL_nomCanoniqueSet, VAL_nomCanoniqueHashSet)) {
									if(VAL_nomCanoniqueBoolean.equals(entiteNomCanoniqueGenerique)) {
										...
										tl(5, "w.s(\"\\\"", entiteVar, "\\\": [\");");
										tl(5, "for(int k = 0; k < entiteValeurs.size(); k++) {");
										tl(6, "entiteValeur = entiteValeurs.get(k);");
										tl(6, "if(k > 0)");
										tl(7, "w.s(\", \");");
							...
							if(classeSauvegarde) {
								...
								if(BooleanUtils.isTrue(entiteAttribuer)) {
									...
									if(StringUtils.equals(entiteNomCanonique, List.class.getCanonicalName()) || StringUtils.equals(entiteNomCanonique, ArrayList.class.getCanonicalName())) {
										...
										tl(tBase + 2, "case \"add", entiteVarCapitalise, "\":");
										tl(tBase + 3, "requeteJson.getJsonArray(methodeNom).forEach((v) -> {");
			...
			for(String classeApiMethode : classeApiMethodes) {
				...
				if(classePageLangueNom == null || classePageLangueNom.equals(langueNom)) {
					...
					if(classeApiMethode.contains("POST")) {
						...
						tl(11, "if(connexionSql == null) {");
						tl(12, "gestionnaireEvenements.handle(Future.succeededFuture(d.result()));");
						tl(11, "} else {");
						tl(12, "connexionSql.commit(e -> {");
						tl(13, "if(e.succeeded()) {");
						tl(14, "connexionSql.close(f -> {");
						tl(15, "if(f.succeeded()) {");
						tl(16, "gestionnaireEvenements.handle(Future.succeededFuture(d.result()));");
						tl(15, "} else {");
						tl(16, "erreur", classeNomSimple, "(requeteSite, gestionnaireEvenements, f);");
						tl(15, "}");
						tl(14, "});");
						tl(13, "} else {");
						tl(14, "erreur", classeNomSimple, "(requeteSite, gestionnaireEvenements, e);");
						tl(13, "}");
						tl(12, "});");
						tl(11, "}");
						tl(10, "} else {");
...
//							tl(3, "page.setPageUrl(\"", siteUrlBase, classeApiUri, "\");");
		...
		//					tl(4, "}");
			...
			tl(6, "SQLConnection connexionSql = sqlAsync.result();");
			tl(6, "connexionSql.setAutoCommit(false, a -> {");
			tl(7, "if(a.succeeded()) {");
			tl(8, "requeteSite.setConnexionSql(connexionSql);");
			tl(8, "gestionnaireEvenements.handle(Future.succeededFuture());");
			tl(7, "} else {");
			tl(8, "gestionnaireEvenements.handle(Future.failedFuture(a.cause()));");
			tl(7, "}");
			tl(6, "});");
			tl(5, "} else {");
			tl(6, "gestionnaireEvenements.handle(Future.failedFuture(sqlAsync.cause()));");
			tl(5, "}");
			tl(4, "});");
			tl(3, "}");
			tl(2, "} catch(Exception e) {");
...
//					tl(4, "entiteValeur = Optional.ofNullable(documentSolr.getFieldValues(", q(entiteVar, "_stored", entiteSuffixeType), ")).map(Collection<Object>::stream).orElseGet(Stream::empty).findFirst().orElse(null);");
			...
			tl(9, ", new JsonArray(Arrays.asList(", q(classePartsUtilisateurSite.nomCanonique), ", utilisateurId))");
			tl(9, ", creerAsync");
			tl(7, "-> {");
